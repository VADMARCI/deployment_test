...
func (m *Plugin) generatePerSchema(data *codegen.Data) error {
	...
	for _, o := range objects {
		isEntity := o.Name == "Entity"

		if o.HasResolvers() {
			fn := gqlToResolverName(
				data.Config.Resolver.Dir(), o.Position.Src.Name, data.Config.Resolver.FilenameTemplate,
			)
			if files[fn] == nil {
				files[fn] = &File{}
			}
		...
		for _, f := range o.Fields {
			hasHooDirective := false
			isEnum := f.TypeReference.Definition.Kind == ast.Enum
			var hooDirective *codegen.Directive
			if f.HasDirectives() {
				directives := f.Directives
				for _, d := range directives {
					if d.Name == "hooResolver" {
						hasHooDirective = true
						hooDirective = d
						break
					}
			...
			if !f.IsResolver && !hasHooDirective {
				continue
			}
			...
			var action string
			if hooDirective != nil {
				for _, arg := range hooDirective.Args {
					switch arg.Name {
					case "resolver":
						action = arg.Value.(string)
					}
				}
				implementation = fmt.Sprintf("return %s", action)
			} else if isEntity {
				implementation = fmt.Sprintf("return genresolvers.%s(ctx,id)", f.GoFieldName)
			} else if isEnum {
				implementation = fmt.Sprintf(
					"panic(fmt.Errorf(\"Enum not implemented: %s  %v - %v\"))", o.Name, f.GoFieldName,
					f.Name,
				)
			} else {
				implementation = fmt.Sprintf(
					"panic(fmt.Errorf(\"overwrote implemented: %v - %v\"))", f.GoFieldName,
					f.Name,
				)
			}
			// }
			...
			fn := gqlToResolverName(
				data.Config.Resolver.Dir(), f.Position.Src.Name, data.Config.Resolver.FilenameTemplate,
			)
			if files[fn] == nil {
				files[fn] = &File{}
			}
	...
	for filename, file := range files {
		...
		file.imports = append(file.imports, rewrite.Import{ImportPath: "dealership/presenters/graph_admin/genresolvers"})
		file.imports = append(file.imports, rewrite.Import{ImportPath: "dealership/resolvers"})
