...
func (cc *CarCreate) SetIsSold(b bool) *CarCreate {
	cc.mutation.SetIsSold(b)
	return cc
}

// SetNillableIsSold sets the "is_sold" field if the given value is not nil.
func (cc *CarCreate) SetNillableIsSold(b *bool) *CarCreate {
	if b != nil {
		cc.SetIsSold(*b)
	}
	return cc
...
func (cc *CarCreate) defaults() {
	if _, ok := cc.mutation.IsSold(); !ok {
		v := car.DefaultIsSold
		cc.mutation.SetIsSold(v)
	}
}
...
func (cc *CarCreate) check() error {
	if _, ok := cc.mutation.IsSold(); !ok {
		return &ValidationError{Name: "is_sold", err: errors.New(`ent: missing required field "Car.is_sold"`)}
	}
...
func (cc *CarCreate) createSpec() (*Car, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.IsSold(); ok {
