...
func (c *CarQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "isSold":
			if _, ok := fieldSeen[car.FieldIsSold]; !ok {
				selectedFields = append(selectedFields, car.FieldIsSold)
				fieldSeen[car.FieldIsSold] = struct{}{}
			}
		case "name":
		...
		case "price":
			if _, ok := fieldSeen[car.FieldPrice]; !ok {
				selectedFields = append(selectedFields, car.FieldPrice)
				fieldSeen[car.FieldPrice] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
